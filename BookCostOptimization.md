...
- **Status (2026-08-04): routing infrastructure built, experiment not yet run.** `model_routing.model_for_step(step, image_model, scene_image_model)` resolves which model each pipeline step uses; `generate_reference_images` gained an optional `cartoon_model` param, `story_service.generate_book`/`/generate` gained an optional `scene_image_model` field (frontend: a "Scene Image Model (experimental cost test)" dropdown, mixed-media only, defaulting to "Same as Image Model" — i.e. today's single-model behavior, unchanged unless explicitly overridden). This is the routing plumbing only — the actual Pro-ref-then-Flash-everything-else quality verdict below still needs a human to run and judge; nothing here claims a savings result yet.
